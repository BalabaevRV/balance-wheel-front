...
export const WheelsListPage = () => {
  ...
  const wheelsList = wheels.map((wheel) => (
    <WheelCard key={wheel.wheel_id} name={wheel.name} fields={wheel.fields} wheelId={wheel.wheel_id} ownerButton={wheel.owner_id === id} attachedButton={userWheels.some(w => w.wheel_id === wheel.wheel_id)} />
  )); 
