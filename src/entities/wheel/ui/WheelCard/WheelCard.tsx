...
export const WheelCard = ({ name, fields, wheelId, ownerButton, attachedButton }: IWheelCardProps) => {
    ...
    <li className='bg-blue-300 rounded-xl p-4 w-full sm:w-48 flex flex-col' >
        ...
        <div className='flex-col mt-auto'>
          {attachedButton && (
            <Button className='mb-2 w-full' onClick={createRecord}>{t('createRecord')}</Button>
          )}
          {ownerButton && (
            <Button className='mb-2 w-full' onClick={editWheel}>{t('editWheel')}</Button>
          )}
          {!ownerButton && attachedButton && <Button className='mb-2 w-full' onClick={detachWheel}>{t('detachWheel')}</Button>}
          {!ownerButton && !attachedButton && <Button className='mb-2 w-full' onClick={attachWheel}>{t('attachWheel')}</Button>}
        </div>
    </li>
  )
} 
